from flask import Flask # flask.py안에 Flask class 로드
...
# flsk server 보안 규칙
# 1. html문서들은 render_template으로 로드 시
# 반드시 template 폴더 내에 존재해야 한다.
# 2. 모든 경로에 대해 접근 불가
# 단, static 경로만 접근 가능
...
def detect():
    ...
    i = ImageDetect() #클래스는 대문자로, 함수는 소문자로
    ...
    nc = int(result[0][5])
    label = i.data[nc]
    output = '<h1>{}일 확률이 {:.2f}%입니다.</h1>'.format(label, cnf*100) # :.2f => 소수점 2째자리 까지 출력
    return output

    # file관련 경로, 이름들은 보안을 지켜주자
    
    
# 내가 직접 실행(run)시 내자아 변수 __name__이 __main__으로 변한다.
